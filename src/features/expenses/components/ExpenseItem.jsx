...
export function ExpenseItem({ expense, onDeleteExpense }) {
    ...
    return (
        <li className={`flex items-center justify-between p-4 border rounded-xl transition-all duration-300 hover:shadow-lg hover:scale-[1.01] ${colors.card} ${colors.border} shadow-md`}>
            <div className="flex items-center gap-3 flex-1">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-xl ${categoryColor}`}>
                    {categoryIcon}
                </div>
                <div className="flex-1">
                    <h3 className={`text-base font-bold ${colors.text}`}>{expense.title}</h3>
            ...
            <div className="flex items-center gap-4">
                ...
                <Button variant="danger" onClick={handleClickDelete} className="px-3 py-1.5 text-sm">
                    🗑️ Delete
                </Button>
            </div>
        </li>
